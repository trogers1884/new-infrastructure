...
return new class extends Migration
{
    ...
    protected $basename = 'resource_type_mappings';
    ...
    public function upInstructions(): void
    {
        ...
        $tblDef = "CREATE TABLE IF NOT EXISTS {$this->schema}.tbl_{$this->basename}
            (
                ...
                , resource_value_column character varying (255) NOT NULL
                , created_at timestamp(0) with time zone
                , updated_at timestamp(0) with time zone
                , deleted_at timestamp(0) with time zone
                , CONSTRAINT pk_{$this->basename}_resource_type_id PRIMARY KEY (resource_type_id)
                , CONSTRAINT fk_{$this->basename}_resource_types_resource_type_id FOREIGN KEY (resource_type_id)
                    REFERENCES auth.tbl_resource_types (id) MATCH SIMPLE
                    ON UPDATE NO ACTION
                    ON DELETE CASCADE
            )";
        $idxs = ([
            "CREATE UNIQUE INDEX unq_{$this->basename}_table_schema_table_name ON {$this->schema}.tbl_{$this->basename} (table_schema, table_name)"
