...
return new class extends Migration
{
    ...
    protected $basename = 'navigation_items';
    ...
    public function upInstructions(): void
    {
        ...
        $tblDef = "CREATE TABLE IF NOT EXISTS {$this->schema}.tbl_{$this->basename}
            (
                ...
                CONSTRAINT fk_{$this->basename}_menu_types_menu_type_id FOREIGN KEY (menu_type_id)
                    REFERENCES config.tbl_menu_types (id) MATCH SIMPLE
                    ON UPDATE NO ACTION
                    ON DELETE CASCADE,
                CONSTRAINT fk_{$this->basename}_navigation_items_parent_id FOREIGN KEY (parent_id)
                    REFERENCES config.tbl_navigation_items (id) MATCH SIMPLE
                    ON UPDATE NO ACTION
                    ON DELETE CASCADE
            )
            ";
        $idxs = ([
            "CREATE UNIQUE INDEX unq_{$this->basename}_menu_type_id_name ON {$this->schema}.tbl_{$this->basename} (menu_type_id, name)"
